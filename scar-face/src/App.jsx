...
// ─── fetchLivePrices — Vercel /api/prices + Groq fallback ─────────────
async function fetchLivePrices(){
  const now=Date.now();

  // Primary: Vercel serverless → Twelve Data (real futures prices)
  try{
    ...
    const r=await fetch("https://api.groq.com/openai/v1/chat/completions",{
      ...
      body:JSON.stringify({
        ...
        messages:[
          ...
          {role:"user",content:`What are the current approximate prices for WTI Crude Oil futures (CL), Gold futures (GC), and Nasdaq 100 E-mini futures (NQ)? Return ONLY: {"OIL":{"price":71.50,"change":1.25},"GOLD":{"price":2950.0,"change":-0.45},"NQ":{"price":19500.0,"change":-1.96}}`},
        ],
      }),
    });
    if(r.ok){
      ...
      if(match){
        ...
        for(const asset of ["OIL","GOLD","NQ"]){
          ...
          if(d?.price!=null&&d?.change!=null){
            ...
            out[asset]={price:+parseFloat(d.price).toFixed(2),change:+chg.toFixed(2),trend:chg>=0?"bullish":"bearish",live:true,fetchedAt:now};
...
async function fetchNews(){
  ...
  if(FINNHUB_KEY&&FINNHUB_KEY!=="your_finnhub_key_here"){
    try{
      ...
      const seen=new Set();
      const fresh=allNews.filter(a=>{
        if(seen.has(a.headline))return false;
        seen.add(a.headline);
        return(now-a.datetime*1000)<SIX_HOURS;
      }).sort((a,b)=>b.datetime-a.datetime).slice(0,20);
      if(fresh.length>2){
        const news=fresh.map(a=>({title:a.headline||"",link:a.url||"",pubDate:new Date(a.datetime*1000).toISOString(),description:a.summary||"",source:"Finnhub"})).filter(h=>h.title.length>5);
  ...
  try{
    ...
    if(allRss.length>0){
      const seen=new Set();
      const unique=allRss.filter(h=>{const k=h.title.slice(0,40).toLowerCase();if(seen.has(k))return false;seen.add(k);return true;})
        .sort((a,b)=>{const da=safeParsePubDate(b.pubDate),db=safeParsePubDate(a.pubDate);if(!da&&!db)return 0;if(!da)return 1;if(!db)return-1;return da-db;})
        .slice(0,25);
...
async function analyzeMarket({fg,momentum,news,apiKey,prices}){
  ...
  const momText=Object.entries(momentum).map(([k,v])=>`${k}: ${v.change!==null?(v.change>0?"+":"")+v.change+"% today":"no data"} (${v.trend}, ${v.strength})`).join(", ");
...
function useMarket(apiKey){
  ...
  const refresh=useCallback(async()=>{
    ...
    try{
      ...
      addLog("📡 Fetching prices (Twelve Data)...");
      const prices=await fetchLivePrices();
      const priceLog=ASSETS.map(a=>{const p=prices[a.id];if(!p?.price)return`${a.id}:FAIL`;if(p.cached)return`${a.id}:CACHE(${p.change}%)`;return`${a.id}:${p.change>0?"+":""}${p.change}%`;}).join(" ");
      ...
      setState(s=>({...s,fg,momentum,prices,news,newsLive,momentumLive,fgLive:fg.live,pricesLive:Object.values(prices).some(p=>p.live),status:"analyzing",lastNewsUpdate:fetchedAt}));
  ...
  useEffect(()=>{
    if(!apiKey||apiKey.length<=20)return;
    refresh();
    autoRefreshTimer.current=setInterval(()=>{refreshPricesOnly();},CACHE_TTL_MS);
...
// ─── Primitives ───────────────────────────────────────────────────────
const Dot=({color,pulse,size=7})=>(<div style={{width:size,height:size,borderRadius:"50%",background:color,boxShadow:`0 0 ${size+2}px ${color}`,flexShrink:0,animation:pulse?"pdot 1.8s ease-in-out infinite":"none"}}/>);
const Bar=({pct,color,h=5})=>{const w=Math.min(Math.max(pct,0),100);return(<div style={{height:h,background:"rgba(255,255,255,0.05)",borderRadius:h,overflow:"hidden"}}><div style={{height:"100%",width:`${w}%`,background:`linear-gradient(90deg,${color}55,${color})`,borderRadius:h,transition:"width 1.2s ease"}}/></div>);};
const Spinner=({size=18})=>(<div style={{width:size,height:size,border:"2px solid rgba(255,255,255,0.08)",borderTop:"2px solid #3b82f6",borderRadius:"50%",animation:"spin .7s linear infinite",flexShrink:0}}/>);
const ImpactDot=({level})=>{const c=level==="high"?"#ef4444":level==="medium"?"#eab308":"#64748b";return<div style={{width:8,height:8,borderRadius:"50%",background:c,boxShadow:`0 0 6px ${c}`,flexShrink:0}}/>;};
  ...
  return(
    <div style={{background:"rgba(6,8,16,0.97)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:16,display:"flex",alignItems:"center",gap:18}}>
      <svg width="148" height="80" viewBox="0 0 150 80">
        ...
        {["#ef4444","#f97316","#eab308","#22c55e","#16a34a"].map((c,i)=>{const sa=(i*36-90)*Math.PI/180,ea=(i*36+33-90)*Math.PI/180;return<path key={i} d={`M${cx+r*Math.cos(sa)},${cy+r*Math.sin(sa)} A${r},${r} 0 0,1 ${cx+r*Math.cos(ea)},${cy+r*Math.sin(ea)}`} fill="none" stroke={c} strokeWidth="13" opacity="0.2"/>;} )}
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:7,marginTop:3}}>
                ...
                {(price?.live||price?.cached)&&<span style={{fontFamily:F,fontSize:10,color:priceColor}}>{price.trend==="bullish"?"▲":"▼"}{price.change>0?"+":""}{price.change}%{price.cached&&<span style={{fontSize:8,opacity:0.5}}> ↺</span>}</span>}
              </div>
            </div>
          </div>
          {displayBias&&(
            <div style={{background:bm.bg,border:`1px solid ${bm.border}`,borderRadius:8,padding:"5px 12px",flexShrink:0,maxWidth:175}}>
              <div style={{fontFamily:F,fontSize:11,letterSpacing:1,color:bm.color}}>{bm.icon} {source==="live"||source==="cached"||!effectiveBias?"LIVE":"AI"}: {displayBias}</div>
        ...
        <div style={{padding:"0 16px 18px",display:"flex",flexDirection:"column",gap:12,animation:"fadeup .22s ease"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{l:"SENTIMENT",v:data.sentiment_edge},{l:"VS CROWD",v:data.crowd_vs_smart},{l:"BULL LEVEL",v:data.key_level_bull},{l:"BEAR LEVEL",v:data.key_level_bear}].map(f=>{
              ...
              return(<div key={f.l} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.05)",borderRadius:10,padding:"10px 12px"}}><div style={{fontFamily:F,fontSize:9,color:"rgba(255,255,255,0.2)",marginBottom:4}}>{f.l}</div><div style={{fontFamily:F,fontSize:13,color:c||"rgba(255,255,255,0.7)"}}>{f.v}</div></div>);
            })}
          </div>
          {data.smt_note&&<div style={{background:`${smtC}08`,border:`1px solid ${smtC}25`,borderRadius:10,padding:"11px 13px"}}><div style={{fontFamily:F,fontSize:10,color:smtC,marginBottom:4}}>SMT — {data.smt_signal?.toUpperCase()}</div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.65}}>{data.smt_note}</div></div>}
          <div style={{background:`linear-gradient(135deg,${bm.bg},transparent)`,border:`1px solid ${bm.border}`,borderRadius:10,padding:"12px 14px"}}><div style={{fontFamily:F,fontSize:10,color:bm.color,letterSpacing:2,marginBottom:6}}>APPROACH</div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.65)",lineHeight:1.7}}>{data.approach}</div></div>
        </div>
      )}
    </div>
  );
...
function NewsTab({rawNews,newsLive,lastUpdate,apiKey,onRefreshNews}){
  ...
  const handleRefresh=async()=>{setRefreshing(true);await onRefreshNews();setRefreshing(false);if(analysis.status==="done")setAnalysis({status:"idle",data:null,error:null});};
  const handleScan=async()=>{if(!rawNews?.length)return;setAnalysis({status:"loading",data:null,error:null});try{const data=await analyzeNews({news:rawNews,apiKey});setAnalysis({status:"done",data,error:null});}catch(e){setAnalysis({status:"error",data:null,error:e.message});}};
  const filters=["ALL","OIL","GOLD","NQ","🔴 HIGH"];
  const analyzed=analysis.data?.analyzed||[];
  const filtered=analyzed.filter(h=>{if(filter==="ALL")return true;if(filter==="🔴 HIGH")return h.impact_level==="high";return h.assets?.includes(filter);});
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{background:"rgba(5,7,15,0.98)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:16,display:"flex",flexDirection:"column",gap:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><div style={{fontFamily:FB,fontSize:24,letterSpacing:4,color:"#fff",lineHeight:1}}>NEWS SCANNER</div><div style={{fontFamily:F,fontSize:10,color:"rgba(255,255,255,0.25)",marginTop:4}}>{lastUpdate?`Updated ${lastUpdate.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})} EST`:"Not yet loaded"}</div></div>
          <div style={{display:"flex",alignItems:"center",gap:6}}><Dot color={newsLive?"#22c55e":"#475569"} pulse={newsLive} size={6}/><span style={{fontFamily:F,fontSize:9,color:newsLive?"#22c55e":"#64748b",letterSpacing:1}}>{newsLive?"LIVE":"DEMO"}</span></div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          <button onClick={handleRefresh} disabled={refreshing} style={{background:"rgba(59,130,246,0.1)",border:"1px solid rgba(59,130,246,0.25)",borderRadius:10,padding:"12px",color:refreshing?"rgba(255,255,255,0.2)":"#60a5fa",fontFamily:FB,fontSize:16,letterSpacing:2,cursor:refreshing?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>{refreshing?<><Spinner size={14}/> LOADING</>:"🔄 REFRESH"}</button>
          <button onClick={handleScan} disabled={analysis.status==="loading"||!rawNews?.length} style={{background:analysis.status==="loading"?"rgba(255,255,255,0.04)":"linear-gradient(135deg,#f97316,#eab308)",border:"none",borderRadius:10,padding:"12px",color:analysis.status==="loading"?"rgba(255,255,255,0.2)":"#000",fontFamily:FB,fontSize:16,letterSpacing:2,cursor:(analysis.status==="loading"||!rawNews?.length)?"not-allowed":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>{analysis.status==="loading"?<><Spinner size={14}/> SCANNING</>:"📡 AI SCAN"}</button>
        </div>
      ...
      <div style={{background:"rgba(6,8,16,0.97)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"14px 16px",display:"flex",flexDirection:"column",gap:10}}>
        ...
        {(rawNews||[]).map((h,i)=>{const pd=safeParsePubDate(h.pubDate);return(<div key={i} style={{paddingLeft:12,borderLeft:"2px solid rgba(255,255,255,0.07)",display:"flex",flexDirection:"column",gap:3}}><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.55)",lineHeight:1.5}}>{h.title}</div><div style={{display:"flex",gap:8,alignItems:"center"}}>{pd&&<span style={{fontFamily:F,fontSize:9,color:"rgba(255,255,255,0.18)"}}>{pd.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit",timeZone:"America/New_York"})} EST</span>}{h.source&&<span style={{fontFamily:F,fontSize:8,color:"rgba(255,255,255,0.2)",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:4,padding:"1px 5px",letterSpacing:1}}>{h.source.toUpperCase()}</span>}</div></div>);} )}
      </div>
      {analysis.status==="done"&&analysis.data&&(
        <div style={{display:"flex",flexDirection:"column",gap:12,animation:"fadeup .3s ease"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div style={{background:"rgba(239,68,68,0.06)",border:"1px solid rgba(239,68,68,0.15)",borderRadius:12,padding:"12px 14px"}}><div style={{fontFamily:F,fontSize:9,letterSpacing:2,color:"#ef4444",marginBottom:6}}>🎯 TOP RISK</div><div style={{fontFamily:F,fontSize:11,color:"rgba(255,255,255,0.55)",lineHeight:1.6}}>{analysis.data.top_risk}</div></div>
            <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:12,padding:"12px 14px"}}><div style={{fontFamily:F,fontSize:9,letterSpacing:2,color:"#22c55e",marginBottom:6}}>⚡ TOP SETUP</div><div style={{fontFamily:F,fontSize:11,color:"rgba(255,255,255,0.55)",lineHeight:1.6}}>{analysis.data.top_opportunity}</div></div>
          </div>
          <div style={{background:"rgba(59,130,246,0.05)",border:"1px solid rgba(59,130,246,0.15)",borderLeft:"3px solid #3b82f6",borderRadius:12,padding:"12px 14px"}}><div style={{fontFamily:F,fontSize:9,letterSpacing:2,color:"#60a5fa",marginBottom:6}}>📡 MARKET READ</div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.5)",lineHeight:1.7}}>{analysis.data.market_summary}</div></div>
          <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>{filters.map(f=>(<button key={f} onClick={()=>setFilter(f)} style={{background:filter===f?"rgba(59,130,246,0.15)":"rgba(255,255,255,0.03)",border:`1px solid ${filter===f?"rgba(59,130,246,0.4)":"rgba(255,255,255,0.07)"}`,borderRadius:8,padding:"7px 14px",color:filter===f?"#60a5fa":"rgba(255,255,255,0.3)",fontFamily:F,fontSize:10,letterSpacing:1,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{f}</button>))}</div>
          {filtered.map((h,i)=>{const imp=NEWS_IMPACT[h.impact_level]||NEWS_IMPACT.low;const cc=catColor[h.category]||"#64748b";return(<div key={i} style={{background:imp.bg,border:`1px solid ${imp.border}`,borderRadius:14,padding:14,display:"flex",flexDirection:"column",gap:10,animation:"fadeup .25s ease"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6}}><div style={{display:"flex",alignItems:"center",gap:6}}><ImpactDot level={h.impact_level}/><span style={{fontFamily:F,fontSize:9,letterSpacing:2,color:imp.color}}>{imp.label} IMPACT</span></div><span style={{background:`${cc}12`,border:`1px solid ${cc}25`,borderRadius:5,padding:"2px 7px",fontFamily:F,fontSize:8,color:cc,letterSpacing:1}}>{(h.category||"").replace("_"," ").toUpperCase()}</span></div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.75)",lineHeight:1.55}}>{h.title}</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{(h.assets||[]).map(id=>{const ast=ASSETS.find(a=>a.id===id);const dir=h.direction?.[id];const dc=dir==="bullish"?"#22c55e":dir==="bearish"?"#ef4444":"#64748b";return(<div key={id} style={{display:"flex",alignItems:"center",gap:4,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:6,padding:"3px 8px"}}><span style={{fontSize:11}}>{ast?.emoji}</span><span style={{fontFamily:F,fontSize:9,color:dc}}>{dir==="bullish"?"▲":dir==="bearish"?"▼":"→"} {id}</span></div>);})}</div><div style={{fontFamily:F,fontSize:11,color:"rgba(255,255,255,0.42)",lineHeight:1.6,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:8}}>💡 {h.reason}</div></div>);} )}
      ...
      {analysis.status==="idle"&&rawNews?.length>0&&(<div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"22px 16px",textAlign:"center",display:"flex",flexDirection:"column",gap:8,alignItems:"center"}}><div style={{fontSize:30}}>📡</div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.3)",lineHeight:1.7}}>Tap <span style={{color:"#f97316"}}>AI SCAN</span> to analyze headlines.</div></div>)}
    </div>
  );
}
...
function BiasTab({market,prices}){
  ...
  if(!market)return(<div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"28px 16px",textAlign:"center",fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.3)",lineHeight:1.7}}>Tap ↻ on the Intel tab to load analysis first.</div>);
  return(
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {ASSETS.map(a=>{
        ...
        const displayBias=p?.live||p?.cached?(p.trend==="bullish"?"Bullish":"Bearish"):effectiveBias;
        const displaySource=p?.live||p?.cached?(effectiveBias&&!overridden?"ai":"live"):source;
        ...
        if(ub&&d){
          ...
          if(typeof rv==="number"&&typeof tv==="number"){const total=rv+tv;if(total>0){realPct=Math.round((rv/total)*100);trapPct=100-realPct;}else{realPct=50;trapPct=50;}}
        }
        const hasPcts=realPct!==null&&trapPct!==null,isReal=hasPcts&&realPct>=trapPct;
                ...
                {(p?.live||p?.cached)&&(
                  <div style={{display:"flex",flexDirection:"column",gap:4,marginTop:6}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                      <div style={{background:`${priceColor}15`,border:`1px solid ${priceColor}35`,borderRadius:7,padding:"3px 10px"}}><span style={{fontFamily:F,fontSize:11,color:priceColor}}>{p.trend==="bullish"?"▲":"▼"} {p.cached?"CACHED":"LIVE"} {p.change>0?"+":""}{p.change}%</span></div>
                      {overridden&&<div style={{background:"rgba(249,115,22,0.12)",border:"1px solid rgba(249,115,22,0.3)",borderRadius:7,padding:"3px 10px"}}><span style={{fontFamily:F,fontSize:10,color:"#f97316"}}>⚠ AI OVERRIDDEN</span></div>}
                    </div>
                    {overridden&&<div style={{background:"rgba(249,115,22,0.06)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:9,padding:"8px 12px",fontFamily:F,fontSize:11,color:"rgba(255,255,255,0.45)",lineHeight:1.6}}>Price is <span style={{color:priceColor}}>{p.trend.toUpperCase()}</span> — badge updated. AI said {d?.bias}. Hit ↻ to re-analyse.</div>}
                  </div>
                )}
              </div>
              {displayBias&&(<div style={{marginLeft:"auto",background:ebm.bg,border:`1px solid ${ebm.border}`,borderRadius:8,padding:"4px 10px",flexShrink:0,maxWidth:165}}><div style={{fontFamily:F,fontSize:10,color:ebm.color}}>{ebm.icon} {displaySource==="live"||displaySource==="cached"?"LIVE":"AI"}: {displayBias}</div>{overridden&&d?.bias&&<div style={{fontFamily:F,fontSize:9,color:"#f97316",marginTop:1}}>AI: {d.bias} ↺</div>}</div>)}
            </div>
            <KZBadge assetId={a.id}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {["Bullish","Bearish"].map(opt=>{const sel=ub===opt,col=opt==="Bullish"?"#22c55e":"#ef4444";return(<button key={opt} onClick={()=>setUserBias(prev=>({...prev,[a.id]:sel?null:opt}))} style={{background:sel?`${col}18`:"rgba(255,255,255,0.03)",border:`1.5px solid ${sel?col:"rgba(255,255,255,0.08)"}`,borderRadius:12,padding:15,color:sel?col:"rgba(255,255,255,0.28)",fontFamily:F,fontSize:13,letterSpacing:2,cursor:"pointer",transition:"all .18s",display:"flex",alignItems:"center",justifyContent:"center",gap:7}}>{opt==="Bullish"?"▲":"▼"} {opt.toUpperCase()}</button>);})}
            </div>
            {ub&&(
              <div style={{display:"flex",flexDirection:"column",gap:10,animation:"fadeup .25s ease"}}>
                {!hasPcts?(<div style={{background:"rgba(100,116,139,0.07)",border:"1px solid rgba(100,116,139,0.18)",borderRadius:10,padding:"12px 14px",fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.35)",textAlign:"center"}}>No probability data — tap ↻ to refresh.</div>):(
                  <>
                    {[{label:"✅ REAL MOVE",pct:realPct,color:"#22c55e"},{label:"⚡ TRAP RISK",pct:trapPct,color:"#f97316"}].map(b=>(<div key={b.label}><div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:6}}><span style={{fontFamily:F,fontSize:11,letterSpacing:2,color:b.color}}>{b.label}</span><span style={{fontFamily:FB,fontSize:32,color:b.color,lineHeight:1}}>{b.pct}%</span></div><Bar pct={b.pct} color={b.color} h={7}/></div>))}
                    <div style={{background:isReal?"rgba(34,197,94,0.07)":"rgba(249,115,22,0.07)",border:`1px solid ${isReal?"rgba(34,197,94,0.2)":"rgba(249,115,22,0.2)"}`,borderRadius:10,padding:"12px 14px",fontFamily:F,fontSize:12,color:isReal?"#4ade80":"#fb923c",lineHeight:1.7,textAlign:"center"}}>{isReal?`${ub} looks REAL — ${realPct}% real vs ${trapPct}% trap.`:`${ub} looks like a TRAP — ${trapPct}% trap risk vs ${realPct}% real. Wait.`}</div>
                    {showAlignment&&(<div style={{background:aligned?"rgba(34,197,94,0.05)":"rgba(239,68,68,0.05)",border:`1px solid ${aligned?"rgba(34,197,94,0.15)":"rgba(239,68,68,0.15)"}`,borderRadius:10,padding:"10px 13px",fontFamily:F,fontSize:11,color:aligned?"#4ade80":"#f87171",lineHeight:1.6}}>{aligned?`✅ Your bias aligns with the ${displaySource==="live"||displaySource==="cached"?"live price":"AI"} read (${displayBias}).`:`⚠ Your bias (${ub}) conflicts with the ${displaySource==="live"||displaySource==="cached"?"live price":"AI"} read (${displayBias}). High-risk.`}</div>)}
                  </>
                )}
              </div>
            )}
...
function DiagPanel({log,onClose}){
  return(
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(4,5,8,0.99)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:"18px 18px 0 0",padding:"22px 18px 36px",zIndex:400,maxHeight:"55dvh",overflow:"auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><span style={{fontFamily:F,fontSize:12,letterSpacing:2,color:"#60a5fa"}}>LIVE LOG</span><button onClick={onClose} style={{background:"none",border:"none",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:22,padding:"4px 8px"}}>✕</button></div>
      {(log||[]).slice().reverse().map((l,i)=>(<div key={i} style={{fontFamily:F,fontSize:11,color:l.includes("❌")?"#f87171":l.includes("✅")?"#4ade80":l.includes("🤖")?"#a78bfa":"rgba(255,255,255,0.35)",lineHeight:2}}>{l}</div>))}
    ...
    <div style={{minHeight:"100dvh",background:"#030407",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"28px 20px",gap:32}}>
      ...
      <div style={{width:"100%",maxWidth:440,display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,padding:22,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <input type="password" placeholder="gsk_..." value={key} onChange={e=>setKey(e.target.value)} onKeyDown={e=>e.key==="Enter"&&key.length>10&&onSubmit(key)} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:12,padding:"15px 16px",color:"#fff",fontSize:15,width:"100%"}}/>
          <button onClick={()=>key.length>10&&onSubmit(key)} style={{background:key.length>10?"linear-gradient(135deg,#3b82f6,#6366f1)":"rgba(255,255,255,0.04)",border:"none",borderRadius:12,padding:16,color:key.length>10?"#fff":"rgba(255,255,255,0.12)",fontFamily:FB,fontSize:22,letterSpacing:4,cursor:key.length>10?"pointer":"not-allowed",transition:"all .2s"}}>ENTER THE MARKET</button>
        </div>
        <div style={{background:"rgba(59,130,246,0.05)",border:"1px solid rgba(59,130,246,0.15)",borderRadius:14,padding:18}}>
          ...
          <div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.35)",lineHeight:2}}>console.groq.com → Sign up → API Keys<br/><span style={{color:"#22c55e"}}>✓ Free · Fast · 14,400 calls/day</span></div>
        </div>
        <div style={{background:"rgba(249,115,22,0.05)",border:"1px solid rgba(249,115,22,0.15)",borderRadius:14,padding:18}}>
          ...
          <div style={{fontFamily:F,fontSize:11,color:"rgba(255,255,255,0.35)",lineHeight:2}}>Push to GitHub → Import on vercel.com<br/><span style={{color:"#fb923c"}}>Auto-deploys on every git push ✓</span></div>
        </div>
      </div>
    </div>
  );
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(3,4,7,0.97)",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:22,padding:28}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"center",width:"100%",maxWidth:340}}>
        {(log||[]).slice(-4).map((l,i,arr)=>(<div key={i} style={{fontFamily:F,fontSize:11,color:i===arr.length-1?"#60a5fa":"rgba(255,255,255,0.15)",letterSpacing:1,textAlign:"center"}}>{l}</div>))}
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const ny=getNYTime();
  const stMap={idle:{t:"STANDBY",c:"#64748b"},fetching:{t:"FETCHING",c:"#8b5cf6"},analyzing:{t:"ANALYZING",c:"#f59e0b"},live:{t:"LIVE",c:"#22c55e"},error:{t:"ERROR",c:"#f97316"}};
  const{t:stLabel,c:stColor}=stMap[status]||stMap.idle;
  if(!submitted)return(<><style>{CSS}</style><KeyScreen onSubmit={k=>{store.set("sf_key",k);setApiKey(k);_groqKey=k;setSubmitted(true);}}/></>);
  const TABS=[{id:"intel",label:"📊 INTEL"},{id:"news",label:"📰 NEWS"},{id:"bias",label:"⚡ BIAS"},{id:"kz",label:"🕐 KZ"}];
  return(
            ...
            <>
              ...
              {market?(
                <>
                  ...
                  {[{icon:"⚡",label:"PAIR TRADE",text:market.pair_trade,color:"#a855f7"},{icon:"🎯",label:"RISK EVENT",text:market.risk_event,color:"#ef4444"},{icon:"📡",label:"MACRO",text:market.macro_summary,color:"#3b82f6"}].map(c=>(<div key={c.label} style={{background:"rgba(5,7,15,0.98)",border:`1px solid ${c.color}15`,borderLeft:`3px solid ${c.color}`,borderRadius:14,padding:"14px 16px"}}><div style={{fontFamily:F,fontSize:11,letterSpacing:2,color:c.color,marginBottom:7}}>{c.icon} {c.label}</div><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.45)",lineHeight:1.7}}>{c.text}</div></div>))}
                  {market.correlation_warning&&market.correlation_warning!=="null"&&market.correlation_warning!==null&&(<div style={{background:"rgba(249,115,22,0.05)",border:"1px solid rgba(249,115,22,0.18)",borderRadius:14,padding:"12px 16px",display:"flex",gap:10}}><span style={{color:"#f97316",flexShrink:0}}>⚠</span><div style={{fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.42)",lineHeight:1.7}}><span style={{color:"#fb923c"}}>CORRELATION TRAP: </span>{market.correlation_warning}</div></div>)}
                </>
              ):!isLoading&&(<div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"24px 16px",textAlign:"center",fontFamily:F,fontSize:12,color:"rgba(255,255,255,0.3)",lineHeight:1.7}}>{error?`Error: ${error}`:"Tap ↻ to load live analysis"}</div>)}
            </>
          )}
